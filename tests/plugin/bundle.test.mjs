...
import { bundleAll, externals, FAMILIES, repoRoot, version } from '../../scripts/bundle.mjs';
...
/**
 * The banner names the family it built and the version it built.
 *
 * Both were wrong in ways only a second family or a vendored copy would expose. The name was
 * written out as `datavis` regardless of which family was being bundled, so family two would
 * have shipped claiming to be family one. The version was absent altogether: an asset pinned
 * by its tagged URL is identifiable from the URL, but the moment someone copies the file into
 * a repository the URL is gone and nothing in the bytes says what it is.
 *
 * `release.yml` greps for the version line before publishing, so this test and that guard
 * fail together if the stamp is ever dropped.
 */
test('every bundle names its own family and the version it was built from', () => {
  ...
  for (const family of FAMILIES) {
    ...
    assert.equal(
      banner[0],
      `// QuantEcon ${family.name} plugin for mystmd — generated by scripts/bundle.mjs.`,
      `${family.name}.mjs should name itself, not another family`,
    );
    assert.equal(
      banner[1],
      `// version: ${version}`,
      `${family.name}.mjs should carry the package.json version release.yml greps for`,
    );
  }
});
